...
/**
 * Wires up the [dynamic_vaccines]/vaccinepk_render_vaccine_picker() widget
 * (see functions.php): a radio-selected brand per vaccine group needs its
 * value mirrored into that group's shared hidden checkbox (the radios are
 * UI-only — the form only ever sees the hidden checkbox), and the "Other
 * Vaccine" checkbox needs to reveal its free-text input. Delegated on
 * `document` so it covers every .dv-group widget on the page, including
 * ones rendered after page load (e.g. a CF7 AJAX re-render).
 */
function vaccinationCentreInitVaccinePicker() {
    document.addEventListener( 'change', function ( e ) {
        const radio = e.target.closest( '.dv-brand-radio' );
        if ( radio ) {
            const hiddenId = radio.getAttribute( 'data-hidden' );
        ...
        const otherTrigger = e.target.closest( '.dv-other-trigger' );
        if ( otherTrigger ) {
            const targetId = otherTrigger.getAttribute( 'data-target' );
            const wrap     = targetId ? document.getElementById( targetId ) : null;
            if ( wrap ) {
                wrap.style.display = otherTrigger.checked ? 'block' : 'none';
...
document.addEventListener( 'DOMContentLoaded', vaccinationCentreInitVaccinePicker );
